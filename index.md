...
This 📦 includes all the functions needed to analyze distance sampling
data. From [SAMMO-Boat](https://github.com/hytechimaging/sammo-boat) raw
data to distribution mapping and ESW calculation, the pelascope package
brings together a set of useful functions for analyzing field data. A
description is available on the [pelascope
...
Using pelascope requires the installation of packages available in
‘pelaverse’, namely
[pelaCDS](https://gitlab.univ-lr.fr/pelaverse/pelacds) and
[pelaMSFD](https://gitlab.univ-lr.fr/pelaverse/pelaMSFD). It also
requires [Rtools](https://cran.r-project.org/bin/windows/Rtools/) to be
installed on your system.

### {pelascope} installation
...
First step: take care of the effort table. The data set (extracted from
the database) must be imported, and then they let it go!
...
![](reference/figures/README-plot_sub-1.png)

### Check and transform effort

After importing the raw data set from the database, it is necessary to
perform some security checks and transform the data set in order to
continue the workflow. The ‘check_raw_effort’ function allows you to
change the names of the columns, create the DateTime column, and select
the column you are interested in. The function has 3 arguments :
...
In some cases, the LegID column does not represent a constant
environment and may change over time. To resolve this issue, the
‘prep_lin’ function recreates effort points when necessary (with “NEW”
as the status) and adjusts the ‘LegID’ column to obtain a constant
environment during the Leg.
...
Now that the dataset is clean and in the correct format, it is possible
to linearize and segment the effort. The following steps can be
performed using the AmniDSM package
...
#> → 10 Legs have only one point
check$mapfig
```

![](reference/figures/README-AmbiDSM_check-1.png)
...
data_effort_clean_lin = data_effort_clean %>% 
  eff_linearize(variable_keep = c("Beaufort","LegID", "plateform", "n_obs"), 
                length_min = 0.1, 
                crs = 4326)
#> 
#> ── Linearizing Effort ──
#> 
#> ℹ 155 breaks have been made within legs for linearization
#> ℹ 202 Legs have only one point : they are suppressed

head(data_effort_clean_lin)
#> Simple feature collection with 6 features and 10 fields
#> Geometry type: LINESTRING
#> Dimension:     XY
#> Bounding box:  xmin: -4.489055 ymin: 44.06679 xmax: -1.592198 ymax: 47.51919
#> Geodetic CRS:  WGS 84
#> # A tibble: 6 × 11
#>   Beaufort LegID            plateform n_obs    id TransectID Start_time         
#>      <dbl> <chr>            <chr>     <dbl> <dbl> <chr>      <dttm>             
#> 1        1 04052023_C_G3_1… upper_br…     2    28 TR_Pelgas… 2023-05-04 11:42:53
#> 2        1 05052023_C_G12_… upper_br…     2    32 TR_Pelgas… 2023-05-05 13:56:44
#> 3        1 05052023_C_G14_… upper_br…     2    32 TR_Pelgas… 2023-05-05 16:09:17
#> 4        1 18052023_C_G7_7… upper_br…     2   102 TR_Pelgas… 2023-05-18 11:33:05
#> 5        1 27052023_C_G7_1… upper_br…     2   147 TR_Pelgas… 2023-05-27 13:24:10
#> 6        2 01052023_C_G3_4… upper_br…     2     0 TR_Pelgas… 2023-05-01 06:56:56
...
Segmentation of the effort. Here to solve a temporary problem, ‘SegID’
are rendered unique.
...
#> Geometry type: POINT
#> Dimension:     XY
#> Bounding box:  xmin: -2.134607 ymin: 44.46551 xmax: -1.656394 ymax: 44.86573
#> Geodetic CRS:  WGS 84
#> # A tibble: 6 × 11
#>   Beaufort LegID                plateform   n_obs TransectID Start_time         
#>      <dbl> <chr>                <chr>       <dbl> <chr>      <dttm>             
#> 1        1 04052023_C_G3_19_28  upper_brid…     2 TR_Pelgas… 2023-05-04 11:42:53
#> 2        1 05052023_C_G12_22_32 upper_brid…     2 TR_Pelgas… 2023-05-05 13:56:44
#> 3        1 05052023_C_G12_22_32 upper_brid…     2 TR_Pelgas… 2023-05-05 13:56:44
#> 4        1 05052023_C_G12_22_32 upper_brid…     2 TR_Pelgas… 2023-05-05 13:56:44
#> 5        1 05052023_C_G12_22_32 upper_brid…     2 TR_Pelgas… 2023-05-05 13:56:44
#> 6        1 05052023_C_G14_21_32 upper_brid…     2 TR_Pelgas… 2023-05-05 16:09:17
...
![](reference/figures/README-vizualize_effort-1.png)
...
The function ‘check_raw_sightings’ produces a clean sighting dataset and
give some information. Perpendical distance (PerpDist) is calculated
using ‘sinus’ and only appropriate for ship survey.
...
AmbiDSM provides access to another verification that allows you to
obtain up-to-date information. It produces DupID columns: potential
duplicates that can be explored.
...
AmbiDSM has a function that allows you to check which effort data
segments correspond to the TransectID, date and time of each
observation. It then links each observation to the nearest segment and
adds variables describing the observation conditions to the observation
data. The function has several argument :
...
Sigthings_with_SegID <- data_sightings_clean %>% # data sightings clean
  sight_bindSegID(data_effort_clean_Seg , # effort segmented
                  ...
                  return = "sight",
                  remove_dup = FALSE)
#> ✖ 915 sightings removed because observation time not within start/end of effort transects
...
#> → 598 attraction behaviour have been removed from the sighting table.

Sigthings_without_attractor$table_behavior
#> # A tibble: 33 × 3
#> # Groups:   Species, behaviour [33]
#>    Species behaviour      n
#>    <chr>   <chr>      <int>
#>  1 DELDEL  attraction   254
#>  2 LARFUS  attraction    88
#>  3 SULBAS  attraction    63
#>  4 LARGUL  attraction    61
#>  5 FULGLA  attraction    23
#>  6 LARMIC  attraction    17
#>  7 LARARG  attraction    15
#>  8 STRDEC  attraction    14
#>  9 GREGUL  attraction     8
#> 10 HIRRUS  attraction     7
#> # ℹ 23 more rows
Sigthings_without_attractor$table_direction
#> # A tibble: 20 × 3
#> # Groups:   Species, direction [20]
#>    Species direction     n
#>    <chr>       <dbl> <int>
#>  1 PUFPUF        360    10
#>  2 SULBAS        360     7
...
Then to prepare the effort, it is needed to tally the number of
observation per species in each segment (SegID). The function
...
#> 5        1 05052023_C_G12_22_32 upper_brid…     2 TR_Pelgas… 2023-05-05 13:56:44
#> 6        1 05052023_C_G14_21_32 upper_brid…     2 TR_Pelgas… 2023-05-05 16:09:17
#> # ℹ 177 more variables: DateTime <dttm>, End_time <dttm>, SegID <chr>,
...
#>      <dbl> <dbl> <chr>                      <dbl>
#> 1        1     2 upper_bridge_outside        62.4
#> 2        2     2 upper_bridge_outside       523. 
#> 3        3     2 upper_bridge_outside      1488. 
#> 4        4     2 bridge_inside               27.2
#> 5        4     2 upper_bridge_outside      1135. 
#> 6        5     2 bridge_inside              127. 
#> 7        5     2 upper_bridge_outside       307. 
#> 8        6     2 bridge_inside               61.9

descriptive_stats$stack_bar
```

![](reference/figures/README-descriptive_statistics-1.png)
...
![](reference/figures/README-descriptive_statistics-2.png)
...
![](reference/figures/README-descriptive_statistics-3.png)
...
![](reference/figures/README-descriptive_statistics-4.png)
...
![](reference/figures/README-descriptive_statistics-5.png)

## Estimate ESW

Now it’s time to estimate ESW with the function ‘estimate_esw’. From the
table sighting with the SegID column and the effort table just created,
it’s possible to estimate the Effective Strip Width. Species has to be
selected and the variable to be considered. The function produces the
effort table with ESW value, a table with ESW values calculated, a graph
and model selection with abundance estimated. Estimation of ESW is based
on half-normal detection function.
...
#> ── Data cleaning ──
#> 
#> There are 94 sightings in the dataset selected.
#> 
#> ── Model selection ──
#> 
...
![](reference/figures/README-esw_Caculation-1.png)

``` r

export_esw$data_esw
#>              plateform      mean         sd      Q_25     Q_97
#> 1        bridge_inside       NaN         NA 0.0000000 0.000000
#> 2 upper_bridge_outside 0.4597752 0.04660809 0.3582992 0.533839

export_esw$model_selection
#>                      model       AIC det_fun
#> 1            1 + plateform -92.26755      hn
#> 2             1 + Beaufort -91.51532      hn
#> 3 1 + Beaufort + plateform -91.34219      hn

export_esw$effort_output
#> Simple feature collection with 357 features and 16 fields
#> Geometry type: POINT
#> Dimension:     XY
#> Bounding box:  xmin: -5.560823 ymin: 43.66589 xmax: -1.253505 ymax: 48.30484
#> Geodetic CRS:  WGS 84
#> # A tibble: 357 × 17
#>    Beaufort TransectID         plateform         n_obs LegID Start_time         
#>       <dbl> <chr>              <chr>             <dbl> <chr> <dttm>             
#>  1        1 TR_Pelgas_04052023 upper_bridge_out…     2 0405… 2023-05-04 11:42:53
#>  2        1 TR_Pelgas_05052023 upper_bridge_out…     2 0505… 2023-05-05 13:56:44
...
#>  6        1 TR_Pelgas_05052023 upper_bridge_out…     2 0505… 2023-05-05 16:09:17
#>  7        1 TR_Pelgas_18052023 upper_bridge_out…     2 1805… 2023-05-18 11:33:05
#>  8        1 TR_Pelgas_27052023 upper_bridge_out…     2 2705… 2023-05-27 13:24:10
#>  9        2 TR_Pelgas_01052023 upper_bridge_out…     2 0105… 2023-05-01 06:56:56
...
GAM with ‘mgcv’ can be done using soap smooth to take into account for
border effect. The function ‘make_soap’ can do it.
...
![](reference/figures/README-create_soap-1.png)
...
#>  [8] -1.200391 -1.220312 -1.245215 -1.189062 -1.149072 -1.081006 -1.043241
#> [15] -1.065659 -1.169971 -1.195996 -1.209961 -1.114355 -1.089402 -1.174632
...
#> [57] -4.584717 -4.719385 -4.748535 -4.762500 -4.761131 -4.792350 -5.297184
#> [64] -5.793480 -4.152656 -3.137817 -1.792840 -1.512582 -1.484863 -1.178320
#> [71] -1.213574 -1.280273 -1.368701 -1.388867 -1.388672 -1.285059 -1.178320
#> 
#> [[1]]$lat
#>  [1] 43.56377 44.02021 44.55986 44.66182 44.67644 44.72851 44.76401 44.72646
#>  [9] 44.68662 44.66670 45.16147 45.34263 45.53242 45.51203 45.63284 45.68594
#> [17] 45.71445 45.77090 45.76851 45.76023 46.21271 46.33198 46.35010 46.51484
...
#> [57] 48.35752 48.36313 48.41001 48.45024 48.45319 48.45917 48.39700 46.91195
#> [65] 44.88737 43.55620 43.50822 43.54005 43.56377 45.90405 45.81660 45.89712
#> [73] 45.96768 46.03296 46.05039 46.00269 45.90405
head(soap_data$knots)
#>          lon      lat
#> 37 -2.986521 43.73327
#> 38 -2.770601 43.73327
#> 39 -2.554681 43.73327
#> 40 -2.338761 43.73327
#> 41 -2.122841 43.73327
#> 42 -1.906921 43.73327
```
...
#> 
#> 
#> Family: Negative Binomial(0.415) 
#> Link function: log 
#> 
#> Formula:
...
#> FULGLA has a mean density at 0.034 with min= 0 and max= 0.162.
#> $model_data
#> 
#> Family: Negative Binomial(0.415) 
#> Link function: log 
...
![](reference/figures/README-calculation-1.png)
...
![](reference/figures/README-calculation-2.png)

``` R
#> 
#> $data_predict
#> Simple feature collection with 2432 features and 3 fields
#> Geometry type: POLYGON
#> Dimension:     XY
#> Bounding box:  xmin: -5.79348 ymin: 43.53386 xmax: -1.032409 ymax: 48.45917
#> Geodetic CRS:  WGS 84
#> First 10 features:
#>       Density         SE Species                       geometry
#> 4  0.04435625 0.02852029  FULGLA POLYGON ((-5.074828 48.4591...
#> 5  0.04144270 0.02666299  FULGLA POLYGON ((-4.984997 48.4591...
#> 6  0.03812816 0.02413783  FULGLA POLYGON ((-4.895165 48.4591...
#> 7  0.03578681 0.02314304  FULGLA POLYGON ((-4.805334 48.4591...
#> 8  0.05473780 0.03507767  FULGLA POLYGON ((-5.344323 48.3995...
#> 9  0.05106343 0.03181844  FULGLA POLYGON ((-5.254491 48.3995...
#> 10 0.04716910 0.02892573  FULGLA POLYGON ((-5.16466 48.39957...
#> 11 0.04331020 0.02601696  FULGLA POLYGON ((-5.074828 48.3995...
#> 12 0.03969557 0.02368143  FULGLA POLYGON ((-4.984997 48.3995...
#> 13 0.03631750 0.02171854  FULGLA POLYGON ((-4.895165 48.3995...
#> 
#> $data_fit
#> Simple feature collection with 335 features and 18 fields
#> Geometry type: POINT
#> Dimension:     XY
#> Bounding box:  xmin: -5.560823 ymin: 43.66589 xmax: -1.253505 ymax: 48.30484
#> Geodetic CRS:  WGS 84
